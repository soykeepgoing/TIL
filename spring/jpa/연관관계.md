...
* **데이터베이스 세계**
  ...
  * 한쪽 테이블에만 외래키가 존재해도 **양방향 조인**이 가능하다.
...
➡️ 따라서 JPA에서는 이 두 세계의 패러다임 차이를 해결하기 위해 **연관관계 매핑**이 필요하다.
...
* SQL 조인을 작성하지 않아도 **객체 그래프 탐색**으로 자연스럽게 관계를 추적할 수 있다.

---

## 3. 연관관계의 주인 (Owning Side)

* **실제 외래키를 가진 엔터티**가 연관관계의 주인이다.
* 주인만이 외래키 값을 **등록·수정할 수 있다.**
* 반대편은 **조회용(read-only)** 으로만 사용된다.

이는 데이터 값 자체의 변경이 아니라 **관계(연결)의 변경**을 의미한다.
...
* 불필요한 의존 관계를 줄인다.
* 구조가 단순하고 명확하다.
* 필요 시 `mappedBy`를 추가해 **양방향으로 확장 가능**하다.
...
* `User`는 `posts` 컬렉션으로 자신이 쓴 게시글을 알고,
* `Post`는 `user` 필드로 작성자를 안다.
...
1. DB에서는 `post.user_id` 값만 바꾸면 끝이지만,
2. 자바 객체에서는 **양쪽 참조 모두 수정**해야 일관성이 유지된다.
...
JPA의 영속성 컨텍스트는 **객체 상태를 기준으로 동기화**하기 때문에
...
| ------- | ------------------------------------------ |
| **단방향** | N쪽만 1쪽을 참조                                 |
| **양방향** | N쪽은 1쪽을 참조, 1쪽은 `mappedBy`로 N쪽 컬렉션을 조회용 보유 |
...
* 내부적으로 `JOIN FETCH`와 동일한 쿼리를 생성한다.
...
| 구분      | 핵심 포인트                       |
| ------- | ---------------------------- |
| 연관관계 주인 | 외래키를 가진 쪽 (N쪽)               |
| 단방향     | 관계 단순, 한쪽만 탐색                |
| 양방향     | 양쪽 탐색 가능, 양쪽 참조 동기화 필요       |
| N+1 문제  | Lazy 로딩 시 다중 쿼리 발생           |
| 해결책     | Fetch Join / Entity Graph 사용 |